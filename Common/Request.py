...
class Request:
    ...
    def get_request(self, url, data, header):
        ...
        time_total = response.elapsed.total_seconds()   # 接口的响应时间，单位是s

        Common.Consts.STRESS_LIST.append(time_consuming)

        response_dicts = dict()   # 将响应的所有数据以键值对的形式存储到字典中
        response_dicts['code'] = response.status_code    # 响应状态码
        try:
            response_dicts['body'] = response.json()     # .json()方法来解析返回的JSON数据，body为所有的返回数据，以字典的形式存在，可以用get来获取到json里面的某个具体字段
        except Exception as e:
            print(e)
            response_dicts['body'] = ''
